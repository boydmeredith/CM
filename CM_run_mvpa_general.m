function [res, results]= CM_run_mvpa_general(subj_array, task, saveName, portion)
for subNo=(1:length(subj_array))
    
    %% load general parameter information
    tic;
    [S idxTr idxTe par] = CM_mvpa_params(subj_array(subNo), task);
    if nargin > 3
        S.portion = portion;
    else
        S.portion = [];
    ...
    if (S.use_premade_workspace&&existWorkspace) % if we are supposed to use premade workspace, and one with the correct name exists
        ...
        [subj] = JR_mvpa_load_and_preprocess_raw_data(S);
    end
...
%             end
        ...
        %store results
        res.subj{subNo}.penalty(1).nVox(1).weights(1).iter{n} = results;
        res.subj{subNo}.penalty(1).nVox(1).weights(1).S = S;
